...
void MegaTC(Kart::Movement& movement, int frames, int unk0, int unk1) {
    if(System::sInstance->IsContext(PULSAR_MEGATC) && (RKNet::Controller::sInstance->GetConnectionState() == RKNet::CONNECTIONSTATE_ROOM || Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_NEXUS, TOGGLE_MEGA_TC) == 1)) movement.ActivateMega();
